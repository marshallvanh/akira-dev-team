...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    const storiesWithProgress = stories.map(story => ({
      ...
      isCompleted: story.progress[0]?.completed || false,
      isUnlocked: story.week === 1 || (story.progress[0]?.completed || false),
    }));
...
router.get('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    res.json({
      ...
      coverEmoji: story.coverEmoji,
      isCompleted: story.progress[0]?.completed || false,
      lastReadPosition: story.progress[0]?.lastReadPosition || 0,
    });
  } catch (error) {
...
---EXECUTION PLAN---
COMMANDS:
cd /home/marshall/projects/brainy-builds/backend && npx tsc && node dist/server.js

FILES TO CREATE/MODIFY:
app/(main)/stories/index.tsx - Story library list screen
app/(main)/stories/[id].tsx - Individual story reading screen with TTS
---END PLAN---

✅ **Stories API tested and running**

Built:
- Stories list endpoint working
- Individual story endpoint with progress tracking
- Progress update endpoint functional

Next: Building frontend story screens

---EXECUTION PLAN---
FILES TO CREATE:
1. app/(main)/stories/index.tsx - Story library grid
2. app/(main)/stories/[id].tsx - Story reader with text-to-speech
---END PLAN---
...
router.get('/week/:weekNumber', async (req, res) => {
  try {
    ...
    if (!story) {
      return res.status(404).json({ error: 'Story not found for this week' });
    }

    res.json(story);
  } catch (error) {
    console.error('Error fetching story by week:', error);
    res.status(500).json({ error: 'Failed to fetch story' });
  }
});
...
router.post('/:id/progress', async (req, res) => {
  try {
    const storyId = parseInt(req.params.id);
    const { userId, completed, readingTime, comprehensionScore } = req.body;

    if (isNaN(storyId) || !userId) {
      return res.status(400).json({ error: 'Invalid story ID or user ID' });
    }
