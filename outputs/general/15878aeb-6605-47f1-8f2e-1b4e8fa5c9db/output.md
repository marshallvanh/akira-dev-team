...
/home/marshall/projects/brainy-builds/backend/src/data/stories.ts
...
# ⚡ PHASE 2: STORIES API ROUTES

Building complete backend API for story browsing, reading sessions, and progress tracking.
...
**API Endpoints:**
1. `GET /api/stories` — List all stories with user progress
2. `GET /api/stories/:id` — Get single story with content and progress
3. `POST /api/stories/:id/session` — Track reading session (page read)
4. `POST /api/stories/:id/complete` — Mark story complete, award points

**Features:**
- Auto-unlock stories based on progression
- Track current page per user per story
- Award points on completion
- Record time spent per page
...
**Files to create:**
1. `backend/src/routes/stories.routes.ts` — Express routes
2. `backend/src/controllers/stories.controller.ts` — Business logic
3. `backend/src/services/stories.service.ts` — Database operations
4. Update `backend/src/server.ts` — Register new routes

**Database operations:**
- Check/create UserProgress records
- Track ReadingSession data
- Award points via PointsTransaction
- Calculate unlock status based on previous completions

---

**Reply YES to build Phase 2 API routes**

💰 *Cost: $12.1605 (164 calls this session)*