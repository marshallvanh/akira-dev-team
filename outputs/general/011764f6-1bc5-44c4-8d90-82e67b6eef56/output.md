...
cd /home/marshall/projects/brainy-builds/backend && npx prisma db push
---END PLAN---
...
1. **Using `prisma migrate deploy`** instead of `migrate dev`
   - This is non-interactive and works in automated environments
   - Applies existing migrations without prompting

2. **Running `prisma db push`** to sync schema
   - Forces schema changes to database
   - Ensures Prisma Client is regenerated with new models
...
**After this succeeds:**
- Database will have Story and StoryProgress tables
- Prisma Client will recognize `prisma.story` and `prisma.storyProgress`
- We can then run the seed script successfully

Ready to execute these commands.

💰 *Cost: $7.2559 (107 calls this session)*

⚡ *Ready to run*

*What this does:* Apply database migrations and push schema changes for Brainy Builds backend.
