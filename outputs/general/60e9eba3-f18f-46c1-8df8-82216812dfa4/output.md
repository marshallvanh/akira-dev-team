...
Marshall, I'm building a complete authentication system for brainy-builds with Supabase.
...
- `app/(auth)/sign-in.tsx` - Login screen with email/password
- `app/(auth)/sign-up.tsx` - Registration screen

**4. Protected Navigation** (`app/_layout.tsx`)
...
4. Enable Email provider in Supabase Auth settings
...
/home/marshall/projects/brainy-builds/contexts/AuthContext.tsx
import React, { createContext, useContext, useEffect, useState } from 'react';
...
/home/marshall/projects/brainy-builds/app/(auth)/sign-in.tsx
...
/home/marshall/projects/brainy-builds/app/(auth)/sign-up.tsx
import React, { useState } from 'react';
import {
  View,
  Text,
...
Marshall, I see the request. Let me build a complete, production-ready authentication system for brainy-builds.

**WHAT I'M BUILDING:**

1. **Supabase Client** with secure storage
2. **Auth Context** for session management
3. **Login Screen** with email/password
4. **Sign Up Screen** with email/password
5. **Auth Stack Navigator**
6. **Root Layout** with auth protection
...
AuthProvider loads session from SecureStore
  ↓
If session exists → Navigate to (tabs)
If no session → Navigate to (auth)/login
  ↓
User signs in → Session saved → Auto-redirect to tabs
User signs out → Session cleared → Auto-redirect to login
```
...
if (!supabaseUrl || !supabaseAnonKey) {
  throw new Error('Missing Supabase environment variables. Create a .env file with EXPO_PUBLIC_SUPABASE_URL and EXPO_PUBLIC_SUPABASE_ANON_KEY');
}
...
/home/marshall/projects/brainy-builds/app/(auth)/signup.tsx
import { useState } from 'react';
import { View, Text, TextInput, TouchableOpacity, StyleSheet, KeyboardAvoidingView, Platform, Alert } from 'react-native';
import { useAuth } from '@/contexts/AuthContext';
import { Link, useRouter } from 'expo-router';
