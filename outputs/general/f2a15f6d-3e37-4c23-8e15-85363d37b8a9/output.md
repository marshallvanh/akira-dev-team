...
# 🎯 STAGE 21: Frontend Migration - Remove Supabase, Connect to Backend

Marshall, the backend is running and verified. Now we migrate the frontend to use our Express API instead of Supabase.
...
### Files to Update
1. **lib/api.ts** - Point to backend instead of Supabase
2. **lib/AuthContext.tsx** - Use JWT tokens instead of Supabase auth
3. **package.json** - Remove Supabase dependency
4. **app/(auth)/login.tsx** - Create login screen
5. **app/(auth)/register.tsx** - Create registration screen

### Files to Delete
- Remove any Supabase client files
...
```typescript
// 1. NEW lib/api.ts - Backend API client
import AsyncStorage from '@react-native-async-storage/async-storage';
...
// 2. NEW lib/AuthContext.tsx - JWT-based auth
// 3. NEW app/(auth)/login.tsx - Login screen
// 4. NEW app/(auth)/register.tsx - Registration screen
// 5. UPDATE package.json - Remove Supabase
```
...
1. Create new API client with JWT support
2. Rewrite AuthContext to use backend auth
3. Build login/register screens
4. Remove Supabase package
5. Update app navigation to handle auth flow
6. Test end-to-end login → profile setup → main app
...
- ✅ Complete auth flow (register → login → profile → app)
- ✅ AsyncStorage for token persistence

---

Say **yes** to build the frontend migration, or **skip** if you want to see the full file changes first.

💰 *Cost: $4.3876 (65 calls this session)*

---

# BACKEND

# 🎯 STAGE 21: Frontend API Integration - Remove Supabase, Connect to Backend

Marshall, backend is running and tested. Now we connect the frontend to use our Express/Prisma backend instead of Supabase.

---

## WHAT WE'RE DOING

1. **Remove Supabase** - Delete client, update package.json
2. **Update API lib** - Point to localhost:3000 backend
3. **Rewrite AuthContext** - Use JWT tokens instead of Supabase session
4. **Update .env** - Add backend API URL
5. **Test login flow** - Verify end-to-end authentication
...
### Update
- `lib/api.ts` - Change base URL, add auth headers
- `lib/AuthContext.tsx` - JWT-based authentication
- `.env` - Add `EXPO_PUBLIC_API_URL`
...
---EXECUTION PLAN---
COMMANDS:
rm -f /home/marshall/projects/brainy-builds/lib/supabase.ts
...
✅ **Removes Supabase** - Deletes client file, removes from package.json  
✅ **API Integration** - All requests go to Express backend  
✅ **JWT Auth** - Tokens stored in AsyncStorage  
✅ **Auto-login** - Checks token on app start  
✅ **Clean logout** - Clears tokens properly  

---

## NEXT STEPS AFTER THIS

1. **Install dependencies** - `npm install` (to update lockfile)
2. **Start Expo** - Test login flow
3. **Create test account** - Register from app
4. **Verify backend** - Check server logs for requests

---

Say **yes** to remove Supabase and connect to the backend.

💰 *Cost: $4.4556 (65 calls this session)*