...
Looking at my previous execution plan, I had all the files ready but the plan got cut off before `audioGenerator.js` was complete. Let me provide the complete execution plan now.
...
FILES:
~/projects/school-builder/worker/package.json
{
  "name": "school-builder-worker",
  "version": "1.0.0",
  "description": "Content generation worker for school-builder",
  "main": "index.js",
  "scripts": {
    "start": "node index.js",
    "generate": "node -e \"require('./scheduler').runNow()\""
  },
  "dependencies": {
...
~/projects/school-builder/worker/index.js
...
~/projects/school-builder/worker/scheduler.js
...
~/projects/school-builder/worker/notify.js
...
~/projects/school-builder/worker/supabase/client.js
...
~/projects/school-builder/worker/generators/storyGenerator.js
...
~/projects/school-builder/worker/generators/questionGenerator.js
const axios = require('axios');
const supabase = require('../supabase/client');

const BUILDINGS = [
  'Library', 'Science Lab', 'Art Studio', 'Sports Field', 'Music Room',
  'Cafeteria', 'Computer Lab', 'Garden', 'Gymnasium', 'Theater'
];
...
Available buildings: ${BUILDINGS.join(', ')}
...
~/projects/school-builder/worker/generators/audioGenerator.js
const axios = require('axios');
const supabase = require('../supabase/client');
...
### 5. `generators/storyGenerator.js`
...
### 6. `generators/questionGenerator.js`
...
### 7. `generators/audioGenerator.js`
...
async function generate(story) {
  ...
  for (let chapter of story.chapters) {
    console.log(`[AUDIO] Generating audio for Chapter ${chapter.chapter_number}: ${chapter.title}`);
    ...
    const { data: audioFile, error: audioError } = await supabase.storage
      .from('audio-files')
      .upload(`chapters/${chapter.id}.mp3`, audioBuffer, {
        contentType: 'audio/mpeg',
        upsert: true
      });

    if (audioError) {
      throw new Error(`Failed to upload audio for chapter ${chapter.chapter_number}: ${audioError.message}`);
    }

    console.log(`[AUDIO] Uploaded audio for Chapter ${chapter.chapter_number}: ${audioFile.Key}`);

    // Update chapter with audio URL
    const { error: updateError } = await supabase
      .from('chapters')
      .update({ audio_url: audioFile.Key })
      .eq('id', chapter.id);

    if (updateError) {
      throw new Error(`Failed to update chapter audio URL: ${updateError.message}`);
    }

    console.log(`[AUDIO] Updated chapter audio URL for Chapter ${chapter.chapter_number}`);
  }
}
