...
function buildPlanningPrompt(state, task, existingFiles, projectSlug) {
  ...
  return `
You are the Frontend Worker in an AI dev team.
...
Rules:
- stay focused on frontend work only
...

{
  ...
  "implementation_notes": "clear explanation of the frontend work completed",
  "files": [
    {
...
function fallbackPlan(task) {
  return {
    objective: `Complete ${task?.id || "the task"} frontend work.`,
    files_to_create: [],
    files_to_modify: [],
    dependencies: [],
    ...
    approach: [
      ...
      "Create the required frontend files",
...
async function runTask(task) {
  ...
  if (typeof akira.setFocus === "function") {
    akira.setFocus(`Frontend: ${task.title}`);
  }

  if (typeof akira.setNextAction === "function") {
    akira.setNextAction(`Frontend Worker is planning ${task.id}`);
  }
  ...
  if (typeof akira.setNextAction === "function") {
    akira.setNextAction(`Frontend Worker is building ${task.id}`);
  }
  ...
  try {
    ...
    try {
      ...
      parsed = {
        summary: "Frontend task processed",
    ...
    if (artifactsRequired && writtenFiles.length === 0) {
      const enforcementReport = [
        ...
        `IMPLEMENTATION:\nTask appears to require real frontend artifacts, but no files were returned by the worker.`,
        `FILES:\nNo files created`,
        `NEXT STEP:\nResubmit with actual frontend files included in the files array.`,
        `BLOCKERS:\nArtifact enforcement: required files were not produced`
      ].join("\n\n");

      if (typeof akira.addWorkerReport === "function") {
        akira.addWorkerReport(task.id, `[Frontend Worker]\n\n${enforcementReport}`, "Frontend Worker");
      }
    ...
    if (typeof akira.addWorkerReport === "function") {
      akira.addWorkerReport(task.id, `[Frontend Worker]\n\n${report}`, "Frontend Worker");
    }

    if (typeof akira.completeTask === "function") {
      akira.completeTask(task.id, `[Frontend Worker]\n\n${report}`);
    }

    return {
      ok: true,
      taskId: task.id,
      output: `[Frontend Worker]\n\n${report}`,
      files: writtenFiles.map((file) => path.join(workspaceInfo.slug, file))
    };
  } catch (err) {
    const message = err.message || "Frontend worker execution failed";

    if (typeof akira.addWorkerReport === "function") {
      akira.addWorkerReport(task.id, `[Frontend Worker Error]\n${message}`, "Frontend Worker");
    }
