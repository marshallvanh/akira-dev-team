...
const FILE = path.join(__dirname, "conversation-memory.json");
...
function setActiveIdea(idea, summary = "") {
  ...
  memory.active_idea = String(idea || "").trim();
  memory.active_idea_summary = String(summary || "").trim();
...
function clearAllMemory() {
  return writeMemory(defaultMemory());
}
...
function getRecentContext(limit = 6) {
  const memory = readMemory();

  const items = memory.history.slice(-limit);
